...
import type { HerdrSessionCatalogOperations, HerdrStatusOperations } from "../capabilities/index.js";
...
export class VsCodeHerdrCommands {
  ...
  register(): void {
    ...
    try {
      registrations.push(vscode.commands.registerCommand("herdr.showStatusActions", () => status.showActions()));
      registrations.push(vscode.commands.registerCommand("herdr.retryDiscovery", () => catalog.retry()));
      registrations.push(vscode.commands.registerCommand("herdr.start", () => catalog.start()));
      registrations.push(
